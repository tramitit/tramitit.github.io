...
const geminiModel = genAI.getGenerativeModel({
  ...
  systemInstruction: 'You are the COO of Tramitit.com, a website that collects the governmental procedures and explains through in a given format.',
});
...
async function generateProcedureGemini(service, url) {
  ...
  const result = await retryOperation(() => chatSession.sendMessage('Insert the prompt here'));
      ...
      for (const { service, url } of services) {
        ...
        if (trimmedUrl) {
          ...
          if (!fs.existsSync(filePath)) {
            const procedureMarkdown = await generateProcedure(service, trimmedUrl);
            console.log(`Writing procedure for service: ${service} and URL: ${trimmedUrl}`);
            fs.writeFileSync(filePath, procedureMarkdown);
          } else {
            console.log(`File for service '${service}' already exists, skipping creation.`);
          }
        }
      }
